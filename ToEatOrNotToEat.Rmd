...
```{r}
ViolationsData <- read.csv(file="/Users/jgalsurkar/Documents/Columbia/EDAV/ToEatOrNotToEat/inspection.csv", header=TRUE, sep=",",as.is=TRUE)
```
...
```{r}
inspection_grades <- ViolationsData %>% select(-CAMIS, -DBA, -BUILDING, -STREET, -ZIPCODE, -PHONE, -INSPECTION.DATE, -ACTION, -VIOLATION.CODE, -VIOLATION.DESCRIPTION, -CRITICAL.FLAG, -SCORE, -GRADE.DATE, -RECORD.DATE, -INSPECTION.TYPE)

ggplot(ViolationsData, aes(BORO, fill = GRADE)) + geom_bar(position = 'dodge') 
